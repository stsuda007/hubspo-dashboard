...
def create_pipeline_chart(df):
    ...
    # Add an explicit final check and cast to datetime
    df_won['初回商談実施日'] = pd.to_datetime(df_won['初回商談実施日'], errors='coerce')
    df_won['受注日'] = pd.to_datetime(df_won['受注日'], errors='coerce')

    # Remove any rows where the conversion failed
    df_won = df_won.dropna(subset=['初回商談実施日', '受注日'])
